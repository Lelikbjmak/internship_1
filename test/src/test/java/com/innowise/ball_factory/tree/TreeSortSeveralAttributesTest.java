package com.innowise.ball_factory.tree;
...
import com.innowise.ball_factory.entity.*;
...
import com.innowise.ball_factory.sorts.binary_tree.TreeSort;
import com.innowise.ball_factory.sorts.heap.HeapSort;
import com.innowise.ball_factory.sorts.merge.MergeSort;
import org.junit.jupiter.api.*;

import java.util.*;

public class TreeSortSeveralAttributesTest {
    ...
    public void ascNameAscWeightSort() {
        ...
        service.sort(new TreeSort<>(), BALLS, new BallComparatorBuilder()
    ...
    public void typePriorityToLargeColorCustomPrioritySort() {
        ...
        Comparator<Ball> ballComparator = new BallComparatorBuilder()
                .comparatorChain(new BallTypePriorityComparator(builder, true),
                        new BallColorPriorityComparator(new ColorPriorityBuilder(), Map.of(Color.YELLOW, 1, Color.PINK, 2)));

        SortingService<Ball> service = new SortingService<>();
        service.sort(new TreeSort<>(), BALLS, ballComparator);
        ...
        Assertions.assertTrue(isAscPriorityArray(types, types.size(),
        ...
        colorsPerTypeMap.forEach((type, colorPerType) ->
                Assertions.assertTrue(isAscPriorityArray(colorPerType, colorPerType.size(), colorPriorityMap),
    ...
    public void typePriorityToSmallColorDarkToLightPrioritySort() {
        ...
        TypePriorityBuilder builder = new TypePriorityBuilder();
        Assertions.assertTrue(isDescPriorityArray(types, types.size(),
        ...
        colorsPerTypeMap.forEach((type, colorPerType) ->
                Assertions.assertTrue(isDescPriorityArray(colorPerType, colorPerType.size(),
    ...
    public void ascWeightToSmallColorDarkToLightPrioritySort() {
        ...
        colorsPerTypeMap.forEach((weight, colorPerWeight) ->
                Assertions.assertTrue(isDescPriorityArray(colorPerWeight, colorPerWeight.size(),
                                colorPriorityBuilder.getAscPriorityMap()),
    ...
    private static <T> boolean isAscPriorityArray(List<T> array, int n, Map<T, Integer> priority) {
        if (n == 1 || n == 0) return true;
        boolean compareResult = priority.get(array.get(n - 2))
                .compareTo(priority.get(array.get(n - 1))) < 1;
        return compareResult && isAscPriorityArray(array, n - 1, priority);
    }

    private static <T> boolean isDescPriorityArray(List<T> array, int n, Map<T, Integer> priority) {
        if (n == 1 || n == 0) return true;
        boolean compareResult = priority.get(array.get(n - 2))
                .compareTo(priority.get(array.get(n - 1))) < 1;
        return compareResult && isDescPriorityArray(array, n - 1, priority);
    }
